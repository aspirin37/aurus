  ...
  <div>
    <div class="d-flex mb-3 align-items-end">
      <h1 class="h4 primary--text">
        {{ $t('views.orders.history_by_weeks') }}
      </h1>
      <div class="ml-auto">
        <download-excel
          file-name="history-by-weeks.xlsx"
          url="/order-reports/history-by-weeks-xlsx"
          :params="filters"
        />
        <v-btn
          outlined
...
export default {
  ...
  methods: {
    async getItems() {
      ...
      const { data } = await this.$http.get('order-reports/history-by-weeks', { params: this.filters }).finally(() => {
        this.loading = false;
      });
