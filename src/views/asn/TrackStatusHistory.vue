  ...
  <div>
    <div class="d-flex mb-3">
      <h1 class="h4 primary--text">
        {{ $t('views.track_status_history.track_status_history') }}
      </h1>
      <div class="ml-auto d-flex">
        ...
        >
          ...
          {{ $t('views.track_status_history.back_to_list') }}
        </router-link>
      </div>
    </div>
    <v-data-table
...
export default {
  ...
  data() {
    return {
      items: [],
      headers: [
        { text: this.$t('views.track_status_history.track_status'), value: 'status' },
  ...
  methods: {
    async getItems() {
      ...
      try {
        ...
        this.items = data.trackStatusHistory;
      } finally {
        this.loading = false;
      }
    },
